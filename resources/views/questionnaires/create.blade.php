            ...
            <div class="p-6 w-full sm:p-8 lg:p-10">
                ...
                <form method="POST" action="{{ route('questionnaires.store') }}" enctype="multipart/form-data">
                    ...
                    <div class="mb-6">
                        ...
                        <textarea id="description" name="description" rows="4" class="block p-2.5 w-full text-sm text-gray-900 bg-gray-50 rounded-lg border border-gray-300 focus:ring-purple-500 focus:border-purple-500" placeholder="Write your description here...">{{ old('description') }}</textarea>
                        @error('description')
                        <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    </div>
