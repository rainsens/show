                ...
                <div class="w-full p-4 bg-white border border-gray-200 rounded-lg shadow sm:p-8">
                    <div class="flex items-center justify-between mb-4">
                        <h5 class="text-xl font-bold leading-none text-gray-900 dark:text-white">
                            Team members in Project: <span class="text-purple-700">{{ $project->title }}</span>
                        </h5>
                        <a href="{{ route('projects.members.create', $project) }}" type="button" class="px-3 py-2 text-xs font-medium text-center inline-flex items-center text-white bg-purple-700 rounded-lg hover:bg-purple-800 focus:ring-4 focus:outline-none focus:ring-purple-300">
                                ...
                                <li class="py-3 sm:py-4">
                                    <div class="flex items-center">
                                        <div class="flex-shrink-0">
                                            <img class="w-12 h-12 rounded-full" src="{{ Str::startsWith($member->user->avatar, 'http') ? $member->user->avatar : asset($member->user->avatar) }}" alt="">
                                        </div>
                                        <div class="flex-1 min-w-0 ms-4">
                                            <p class="text-sm font-medium text-gray-900 truncate dark:text-white">
                                                <a href="{{ route('users.show', $member->user_id) }}" class="hover:text-purple-700">
                                                    <span>{{ $member->user->name }}</span>
                                                    <span class="text-gray-500">{{ $member->user->title }}</span>
                                                </a>
                                            </p>
                                        </div>
                                        <div class="inline-flex items-center text-base font-semibold text-gray-900 dark:text-white">
                                            <x-delete :action="route('members.destroy', $member)" />
