    ...
    <div class="flex flex-col justify-center items-center py-8 px-6 mx-auto">
        <div class="bg-white p-6 shadow-md md:rounded-lg border border-gray-100 justify-center items-center w-full rounded-lg lg:flex md:mt-0 lg:max-w-screen-lg xl:p-0">
            <div class="p-6 w-full sm:p-8 lg:p-10">
                <a href="{{ route('projects.links.index', $link->project_id) }}" type="button" class="mb-8 px-3 py-2 text-xs font-medium text-center inline-flex items-center text-gray-900 bg-white rounded-full hover:bg-gray-100 hover:text-purple-700 focus:z-10 focus:ring-4 focus:ring-gray-100 border border-gray-200">
                ...
                <h5 class="mb-10 text-xl font-bold leading-none text-gray-900 dark:text-white">
                    Edit the link for Project: <span class="text-purple-700">{{ $link->project->title }}</span>
                </h5>
                <form method="POST" action="{{ route('links.update', $link) }}" enctype="multipart/form-data">
                    ...
                    <div class="mb-6">
                        ...
                        <input type="text" id="title" name="title" value="{{ $link->title }}" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-purple-500 focus:border-purple-500 block w-full p-2.5" placeholder="Title" />
                        @error('title')
                    ...
                    <div class="mb-6">
                        ...
                        <input type="url" id="link" name="link" value="{{ $link->link }}" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-purple-500 focus:border-purple-500 block w-full p-2.5" placeholder="Link address" />
                        @error('link')
                        <p class="mt-2 text-sm text-red-600 dark:text-red-500">{{ $message }}</p>
                        @enderror
                    </div>
