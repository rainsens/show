    ...
    <div class="flex flex-col justify-center items-center py-8 px-6 mx-auto">
        <div class="bg-white p-6 shadow-md md:rounded-lg border border-gray-100 justify-center items-center w-full rounded-lg lg:flex md:mt-0 lg:max-w-screen-lg xl:p-0">
            <div class="p-6 w-full sm:p-8 lg:p-10">
                <a href="{{ route('projects.members.index', $project) }}" type="button" class="mb-8 px-3 py-2 text-xs font-medium text-center inline-flex items-center text-gray-900 bg-white rounded-full hover:bg-gray-100 hover:text-purple-700 focus:z-10 focus:ring-4 focus:ring-gray-100 border border-gray-200">
                ...
                <h5 class="mb-10 text-xl font-bold leading-none text-gray-900">
                    Add a new member to Project: <span class="text-purple-700">{{ $project->title }}</span>
                </h5>
                <form method="POST" action="{{ route('projects.members.store', $project) }}" enctype="multipart/form-data">
                    @csrf
                    <div class="mb-6">
