<div class="relative overflow-x-auto shadow-md sm:rounded-lg border border-gray-100">
    <div class="p-5 flex items-center justify-between flex-column flex-wrap md:flex-row space-y-4 md:space-y-0 pb-4 bg-white dark:bg-gray-900">
        ...
        <form method="GET" action="{{ route('projects.search') }}" class="w-1/3">
            <label for="keywords" class="mb-2 text-sm font-medium text-gray-900 sr-only">Search</label>
            <div class="relative">
                ...
                <input type="search" id="keywords" name="keywords" class="block w-full p-2 ps-10 text-sm text-gray-900 border border-gray-300 rounded-lg bg-gray-50 focus:ring-purple-500 focus:border-purple-500" placeholder="Keywords..." />
        ...
        <div class="">
            @if(request()->routeIs('projects.index'))
                <a href="{{ route('projects.mine') }}" type="button" class="text-white bg-purple-700 hover:bg-purple-800 focus:ring-4 focus:ring-purple-300 font-medium rounded-lg text-sm px-5 py-2.5 me-2 mb-2 focus:outline-none">
                    Check My Projects
                </a>
            @else
                <a href="{{ route('projects.create') }}" type="button" class="text-white bg-purple-700 hover:bg-purple-800 focus:ring-4 focus:ring-purple-300 font-medium rounded-lg text-sm px-5 py-2.5 me-2 mb-2 focus:outline-none">
                    Add New Project
                </a>
            @endif
                ...
                <td class="flex items-center px-6 py-4 text-gray-900 whitespace-nowrap dark:text-white">
                    <div class="w-52 relative">
                        @if($project->is_private)
                            <div class="absolute top-1 right-1 bg-yellow-400 w-5 h-5 rounded-full">
